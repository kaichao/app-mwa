...
- 原始产品数据（*.dat）
- 单指向、单通道的fits文件：
- 单指向、单通道fits下采样的文件：按时间维度下采样，数据量为原数据的1/4
- 单指向24通道fits文件：
- 最终候选体文件：通过presto消色散、搜索得到的候选体文件
...
波束合成的数据处理过程，涉及到时间、channel、指向三个维度。
...
处理步骤包括：
- 时间维度变换处理：将给定时间区间的原始数据（*.dat），指定指向，按channel维度独立计算，生成给定时间区间内、单指向、单channel的结果文件，以fits格式存储；
- channel维度数据合并：将前一步骤中的同一指向、单channel的fits文件，按24个channel合并，形成同一指向、给定时间区间的结果文件，也以fits格式存储；
- 按指向维度的数据处理：将前一步骤中同一指向的所有时间区间fits文件，放到统一目录中，形成统一指向的中间数据。将该数据输入到presto算法中，做消色散处理、候选体搜索，得到最终的脉冲星搜索结果。

若按以上步骤，依次在3个维度上进行数据处理。则会带来数据量急剧增加带来的问题，导致极高的I/O需求；

可能的方法包括一个批次处理多指向数据，可以有效减少I/O量。但是带来的问题是对中间存储的需求大量增加，如果缺少有效的控制方法，还会带来进一步I/O瓶颈。

即使通过批量处理多指向数据，其I/O需求依然还是远远超出现有各类计算设施的能力，需要设计高效分级存储体系，减少/避免I/O瓶颈。

#### 4.2.1 原始数据预处理设计

对原始数据做预处理，便于后续按时间维、channel维、指向维的数据处理。
