...
def main():
    ...
    volume_low = os.getenv("VOLUME_LOW") or "120000"
    try:
        volume_low = int(volume_low)
    except ValueError:
        sys.stderr.write(f"Error: VOLUME_LOW environment variable '{volume_low}' is not a valid integer, using default\n")
        volume_low = 120000
        
    volume_high = os.getenv("VOLUME_HIGH") or "145000"
    try:
        volume_high = int(volume_high)
    except ValueError:
        sys.stderr.write(f"Error: VOLUME_HIGH environment variable '{volume_high}' is not a valid integer, using default\n")
        volume_high = 145000
    ...
    for hostname, ip_addr, params in hosts:
        ...
        if "/tmp" in params:
            ...
            try:
                ...
                if volume_low <= tmp_value <= volume_high:
                    filtered_hosts.append((hostname, ip_addr, tmp_value))
    ...
    if not filtered_hosts:
        sys.stderr.write(f"Warning: No hosts found with /tmp volume between {volume_low} and {volume_high}\n")

    
    # 获取所有符合条件的主机的IP地址列表
    ip_list = [host[1] for host in filtered_hosts]
    
    # 更新主机的hostname
    # query_db.update_host_hostname(ip_list, "n", cluster)
    
    # 更新主机的group_id
    # query_db.update_host_group_id(ip_list, new_prefix)

    # 分组更新group_id和hostname
    if (group_num = -1):
        group_num = len(ip_list)
    query_db.update_grouped_hosts(ip_list, new_prefix, cluster, group_num)
    
    print(f"Successfully updated hostname and group_id for {len(ip_list)} hosts")
    
    # 处理/tmp值大于VOLUME_HIGH的主机
    if high_volume_hosts:
        # 获取环境变量REDIS_QUEUE
        redis_queue = os.getenv("REDIS_QUEUE") or "QUEUE_HOSTS"
        
            
        print(f"Found {len(high_volume_hosts)} hosts with /tmp value greater than {volume_high}, sending to Redis queue...")
        
        # 使用redis-cli命令发送消息
        for hostname, ip_addr, tmp_value in high_volume_hosts:
            try:
                # 使用redis-cli命令将消息添加到队列，优先级为1，并通过date命令获取当前时间戳
                cmd = f'redis-cli ZADD {redis_queue} 1 "{ip_addr}:$(date +%s)"'
        ...
        print(f"Completed adding {len(high_volume_hosts)} high /tmp value hosts to Redis queue")
