...
if [[ "$FIX_MISSING" == "yes" ]]; then
    ...
    # 获取实际存在的子目录名并存储在数组中
    echo "/local_data_root${LOCAL_RESULT_DIR}/${dataset}"
    ls -d /local_data_root${LOCAL_RESULT_DIR}/${dataset}/p* > $WORK_DIR/custom-out.txt
    existing_dirs=($(ls -d /local_data_root${LOCAL_RESULT_DIR}/${dataset}/p* 2>/dev/null | xargs -n 1 basename))
    ...
    for dir in ${existing_dirs[@]}; do
        # 统计子目录中的文件数量（包括隐藏文件）
        file_count=$(find "/local_data_root${LOCAL_RESULT_DIR}/${dataset}/${dir}" -type f | wc -l)
        small_files=$(find "/local_data_root${LOCAL_RESULT_DIR}/${dataset}/${dir}" -type f -size -100000c)
